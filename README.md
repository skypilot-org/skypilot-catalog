...
V9 is identical to [v8](#schema-v8), with one added column: `MaximumEfaInterfaces` (the
maximum number of AWS Elastic Fabric Adapter network interfaces the instance
type supports; empty/NaN for non-EFA instances). It is populated by the AWS
catalog fetcher and used to size EFA requests on scale-from-zero clusters.
...
| `MaximumEfaInterfaces` | float | Maximum number of AWS EFA network interfaces supported by the instance type (empty/NaN if EFA is not supported). |

### images.csv
Same as [v7](#imagescsv-2) version.

## Schema V8

The catalogs for each cloud in [v8](./catalogs/v8) include the following files:
1. `vms.csv`: the catalog for the VMs, including the instance and the accelerators.
2. `images.csv`: the catalog for the images, which contains the mapping from the SkyPilot image tag to the image ID that can be used to find the image in the clouds. 
